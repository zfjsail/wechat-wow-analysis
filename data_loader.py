...
class InfluenceDatasetWeChat(Dataset):
    def __init__(self, file_dir, embedding_dim, seed, shuffle, label_type, debug, role):
        if role == "train":
            if isfile(join(settings.DATA_DIR, "node_embedding_spectral.npy")):
                embedding = np.load(join(settings.DATA_DIR, "node_embedding_spectral.npy"))
            else:
                embedding = np.empty(shape=(0, 64))
                for emb_i in range(5):
                    data = np.load(join(settings.DATA_DIR, "node_embedding_spectral_{}.npz".format(emb_i)))
            ...
            vertex_features = np.load(os.path.join(settings.DATA_DIR, "user_features.npy"))
        ...
        self.graphs = np.load(os.path.join(file_dir, "{}_adjacency_matrix.npy".format(role)))
        ...
        self.influence_features = np.load(
            os.path.join(file_dir, "{}_influence_features.npy".format(role))).astype(np.float32)
        logger.info("influence features loaded!")

        self.labels = np.load(os.path.join(file_dir, "{}_{}_labels.npy".format(role, label_type)))
        logger.info("labels loaded!")

        self.vertices = np.load(os.path.join(file_dir, "{}_vertex_ids.npy".format(role)))
        ...
        if debug:
            debug_num = 50000
            self.graphs = self.graphs[: debug_num]
            self.influence_features = self.influence_features[: debug_num]
            self.labels = self.labels[: debug_num]
            self.vertices = self.vertices[: debug_num]
...
class InfluenceDatasetOthers(Dataset):
